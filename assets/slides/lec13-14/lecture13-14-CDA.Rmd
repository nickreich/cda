...
author: "Joshua Freeman, Liz Austin"
output:
  pdf_document: 
    includes:
...
$$Y_i ~ Binomial(n, \pi_i) for i=1,...,k$$
$$f_i(Y) = \left[{\begin{array}{c} n\\y\end{array} } \right] \pi_i^y (1-\pi_i)^{n-y}$$

$Z$ is an equal mixture of all $Y_i$ for which $$f(Z) = \sum_{i=1} f(Z) \frac {f_i(Z)}{K} $$
The marginal distribution for a whole populatio with subpopulations that folowspecific distributions.

GLMM -->Mixed effects = both random and fixed effects.  Not for mixture models.

A mixture model is a probabilistic model for representing the presence of subpopulations within the overalll population, i.e.
another way to account for overdispersion.  Generally, data do not $\underline{NOT}$ provide identifieers of a subpopulation membership.  Estimating groups and group membership.  Subgroups are assured known and we want to get at estimates of characteristics for them.



GLMM as a mixture model:  e.g.

##$$Y_{ij} \left[ {\begin{array}{cc} 1&if\ person\ has\ disease\0&otherwise\end{array} } \right] $$



$$Y_{ij} ~ Bernoulli(\pi_i)$$
$$ \pi_i = logit^{-1}(\alpha_i)$$
$$\alpha_i ~ N(0, \sigma_x^2)  $$

The population level distribution of boy/girl babies is a mixture of $Y_{ij}s$ (plural)

This is not an east close-form solultion for the marginal population-level distribution of boys and girls.  We can at least get at expectation and variance.
