...
where $I$ is variance/covariance matrix of estimate using Fischer scoring.
...
In large sample and $x=x_k$, the SE for $$logit(\hat{\pi_k}) $$ is described as: $$ \sqrt {var(\hat\alpha+\hat\beta x_k)}$$.
...
Building: process by which you arrive at a "chosen" model
...
* Interactive/subjective model building process
...
$glm(y \sim x_1+x_2, offset=log(a),family=poisson,...)$ offset is not aleardy on log  scale.
